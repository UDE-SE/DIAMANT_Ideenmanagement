...
public class IdeaAttachmentControllerTest extends RestDocTest {
    ...
    public void saveAttachment() throws Exception {
        ...
        String businessId = "5dc40fccc270c23c1bc43b8a";
        ...
        this.mockMvc.perform(
                multipart("/api/challenge/{challengeId}/idea/{ideaId}/attachment/{attachmentId}", challengeId, ideaId, businessId)
                        .file(multipartFile)
        )
                .andDo(print())
        ...
        Mockito.verify(denyAccessInterceptor, Mockito.only()).interceptUploadAttachmentToIdea(Mockito.eq(challengeId), Mockito.eq(ideaId), Mockito.eq(businessId), Mockito.any(Authentication.class));
        Mockito.verify(gridFsOperations, Mockito.times(1)).store(Mockito.any(InputStream.class), Mockito.eq(fileName), Mockito.eq(contentType), metaDataCaptor.capture());
        Mockito.verify(gridFsOperations, Mockito.times(1)).delete(Mockito.any(Query.class));
        Assertions.assertThat(metaDataCaptor.getValue().get(AttachmentService.META_DATA_FIELD_CHALLENGE_ID)).isEqualTo(challengeId);
        Assertions.assertThat(metaDataCaptor.getValue().get(AttachmentService.META_DATA_FIELD_ATTACHMENT_BUSINESS_ID)).isEqualTo(businessId);
        Assertions.assertThat(metaDataCaptor.getValue().get(AttachmentService.META_DATA_FIELD_IDEA_ID)).isEqualTo(ideaId);
        ...
        Assertions.assertThat(metaDataCaptor.getValue().get(AttachmentService.META_DATA_FIELD_TYPE)).isEqualTo(AttachmentService.AttachmentType.IDEA_ATTACHMENT.toString());
    }
    ...
    public void getAttachment() throws Exception {
        ...
        String dummyIdeaId = "5c617f7c0f4ccc37b4f769fc";

        Document metadata = new Document();
        metadata.append(AttachmentService.META_DATA_FIELD_CHALLENGE_ID, dummyChallengeId);
        metadata.append(AttachmentService.META_DATA_FIELD_IDEA_ID, dummyIdeaId);
        ...
        this.mockMvc.perform(
                get("/api/challenge/{challengeId}/idea/{ideaId}/attachment/{attachmentId}", dummyChallengeId, dummyIdeaId, dummyId)
        )
        .andDo(print())
        .andExpect(status().isOk())
        ...
        Mockito.verify(denyAccessInterceptor, Mockito.only()).interceptViewIdea(Mockito.eq(dummyChallengeId), Mockito.eq(dummyIdeaId), Mockito.any(Authentication.class));
    }
    ...
    public void getAttachmentWithoutUser() throws Exception {
        ...
        Mockito.verify(gridFsOperations, Mockito.times(1)).findOne(Query.query(Criteria.where("_id").is(dummyId)));
        Mockito.verify(gridFsOperations, Mockito.times(1)).getResource(dummyAttachment);
        Mockito.verify(denyAccessInterceptor, Mockito.only()).interceptViewIdea(Mockito.eq(dummyChallengeId), Mockito.eq(dummyIdeaId), Mockito.isNull());
    }
}
